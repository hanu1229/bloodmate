...
export default function UpdatePressureModal(props) {
    ...
    useEffect(() => { 
        ...
        setSystolic(props.rowInfo.systolicValue);
        setDiastolic(props.rowInfo.diastolicValue);
        setPulse(props.rowInfo.pulseValue);
    }, []);
    ...
    const updatePressure = async () => {
        ...
        try {
            ...
            const response = await axios.put(`${serverDomain}/blood/pressure/${props.rowInfo.bloodPressureId}`, info, {withCredentials : true, headers : {Authorization : token}});
    ...
    return (
        <Box sx = {{padding : "12px", display : "flex", flexDirection : "column", flex : 1, height : "100%"}}>
            ...
            <Button onClick = {updatePressure} sx = {{...btnColor, marginTop : "12px"}}>수정하기</Button>
        </Box>
    );
}